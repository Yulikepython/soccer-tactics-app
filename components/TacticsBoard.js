...
export class TacticsBoard {
    ...
    setupInteractionEvents() {
        ...
        if ('ontouchstart' in window) {
            Object.values(events.touch).forEach(([event, handler, options]) => {
                this.svg.addEventListener(event, handler, options);
            });
        }
        // マウスイベント
    ...
    handleTouchStart(e) {
        ...
        // 新しい要素や線の追加
        if (this.isLineMode()) {
            const line = this.addLine(this.currentMode, point.x, point.y);
            this.currentLine = line;
        }
        else if (this.currentMode) {
            this.addElement(this.currentMode, point.x, point.y);
        }
    }
    handleTouchMove(e) {
        e.preventDefault();
...
//# sourceMappingURL=TacticsBoard.js.map